...
void solve(double* A, double* newA){
    double error = 1.0;
    int iter = 0;

#pragma acc data copyin(A[0:size*size], newA[0:size*size])
    while ((error > eps) && (iter < n_iter)) {
        iter++;
        error = 0.0;

#pragma acc parallel loop reduction(max:error) present(A, newA)
...
#pragma acc parallel loop present(A, newA)
        for(int i = 1; i < size - 1; i++){

#pragma acc loop
            for(int j = 0; j < size - 1; j++){
                A[size * i + j] = newA[size * i + j];
            }
        }
    }
